...
from sdmetrics.single_table import GMLogLikelihood, KSTest
...
class HypothesisTests:
    ...
    def sdv_kolmogorov_smirnov(self, verbose=True) -> Any:
        ...
        results = KSTest.compute(self.reference_dataset.unify(), self.registered_dataset.unify())
    ...
    def gaussian_mixture_log_likelihood(self, verbose=True) -> Any:
        ...
        description = "This metric fits multiple GaussianMixture models to the real data and then evaluates the average log likelihood of the synthetic data on them."
        about_str = "\nMethod: {method}\nDescription:{description}"
        about_str = about_str.format(method=method, description=description)
        ...
        results = GMLogLikelihood.compute(self.reference_dataset.unify(), self.registered_dataset.unify())
        return results
    ...
    def logistic_detection(self, verbose=True) -> Any:
        ...
        if (verbose):
            print(about_str)    
        results = LogisticDetection.compute(self.reference_dataset.unify(), self.registered_dataset.unify())
        return results
    ...
    def sd_evaluate(self, verbose=True) -> Any:
        ...
        if (verbose):
            print(about_str)    
        results = evaluate(self.reference_dataset.unify(), self.registered_dataset.unify(), aggregate=False)
        return results
